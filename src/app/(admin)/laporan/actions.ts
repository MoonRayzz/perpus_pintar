  ...
  try {
    ...
    const loans = await prisma.loan.findMany({
      ...
      include: {
        ...
        student: { select: { name: true, nis: true, class: true } }
      },
      orderBy: { borrowedAt: 'desc' }
    });

    // 2. Ambil Data Kunjungan + Feedback
    ...
    return {
      loans: loans.map(l => ({
        "NIS": l.student.nis,
        "Nama": l.student.name,
        "Kelas": l.student.class,
        "Judul Buku": l.book.title,
        "Tgl Pinjam": l.borrowedAt.toLocaleDateString('id-ID'),
        "Tgl Kembali": l.returnedAt?.toLocaleDateString('id-ID') || "-",
        "Status": l.returnedAt ? "Selesai" : "Dipinjam"
      })),
      visits: visits.map(v => ({
        "Tgl Kunjung": v.checkInTime.toLocaleDateString('id-ID'),
        "Nama": v.student.name,
        "NIS": v.student.nis,
      ...
      books: books.map(b => ({
        ...
        "Total Stok": b.stockTotal,
        "Tersedia": b.stock
      }))
    };
  } catch (error) {
    console.error(error);
    return { loans: [], visits: [], books: [] };
  }
}