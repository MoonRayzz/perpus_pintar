...
import { ScanBarcode, QrCode, CheckCircle, Book, History, AlertTriangle, Loader2, User, BookOpen, XCircle, Calendar, ArrowRight, Clock, Info } from "lucide-react";
...
export default function PeminjamanClient({ 
  ...
  // Hitung default tanggal berdasarkan database pengaturan (bukan lagi hardcode 7 hari)
  const getDefaultDueDate = () => {
    const d = new Date();
    d.setDate(d.getDate() + (settings.maxPinjamHari || 7));
  ...
  const handleBorrow = async () => {
    ...
    if (result.success) {
      ...
      setDueDate(getDefaultDueDate()); // Reset mengikuti setting default
      setActiveTab("active");
    } else {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="flex flex-col gap-2">
              ...
              {studentPreview && studentPreview !== "not_found" && (
                <div className="mt-3 p-4 bg-gradient-to-r from-[#f8f9ff] to-white border border-[#c4c6cf] rounded-xl flex flex-col justify-center gap-2 shadow-sm animate-in slide-in-from-top-2 duration-300">
                  ...
                  {/* Indikator Kuota Buku Siswa Berdasarkan Aturan */}
                  <div className="mt-1 pt-2 border-t border-[#c4c6cf] flex justify-between items-center">
                    <span className="text-xs text-[#43474e] font-bold flex items-center gap-1">
                      <Book size={14}/> Sedang Dipinjam:
                    </span>
                    <span className={`text-xs font-black px-2 py-0.5 rounded ${studentPreview._count.loans >= settings.maxBukuSiswa ? 'bg-[#ffdad6] text-[#93000a]' : 'bg-[#dce9ff] text-[#001f3f]'}`}>
                      {studentPreview._count.loans} / {settings.maxBukuSiswa} Buku
                    </span>
                  </div>
                </div>
              )}
            ...
            <div className="mt-8 p-5 bg-gradient-to-br from-[#001f3f] to-[#003366] rounded-xl shadow-lg border border-[#001f3f] animate-in slide-in-from-top-4 fade-in duration-500 text-white">
              <div className="flex flex-col md:flex-row justify-between items-center gap-6">
                ...
                <div className="flex items-center gap-4 md:gap-8 w-full md:w-auto justify-between md:justify-start">
                  ...
                  {/* Tanggal Kembali (Bisa Diedit Manual) */}
                  <div className="flex items-center gap-3">
                    <div className="p-2 bg-white/10 rounded-lg backdrop-blur-sm">
                ...
                <button 
                  ...
                  disabled={isLoading || studentPreview._count.loans >= settings.maxBukuSiswa}
                  className="w-full md:w-auto bg-[#fcd400] text-[#001f3f] hover:bg-white hover:text-[#001f3f] px-8 py-3.5 rounded-lg font-extrabold text-sm flex items-center justify-center gap-2 transition-all shadow-md active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
                >
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-[#eff4ff] border-b border-[#c4c6cf]">
                ...
                {/* Kolom Status (Aktif = Tampilkan Hari/Denda, Riwayat = Kosongkan) */}
                {activeTab === "active" && (
                  <th className="py-3 px-6 font-semibold text-xs text-[#43474e] uppercase tracking-wider">Status Waktu</th>
                )}
            ...
            <tbody className="divide-y divide-[#c4c6cf] text-sm bg-white">
              {displayedLoans.length === 0 ? (
                ...
                displayedLoans.map((loan) => {
                  // Logika cerdas penghitung hari
                  const today = new Date();
                  const due = new Date(loan.dueDate);
                  // Set jam jadi 00:00 biar hitungan hari akurat (tidak terpengaruh jam pinjam)
                  ...
                  const isOverdue = !loan.returnedAt && diffDays < 0;
                  const lateDays = Math.abs(diffDays);
                  const totalDenda = lateDays * (settings.dendaPerHari || 1000);
                  
                  return (
                    <tr key={loan.id} className={`hover:bg-[#f8f9ff] transition-colors ${isOverdue ? 'bg-[#ffdad6]/20' : ''}`}>
                      <td className="py-4 px-6">
                        <p className="text-[#0b1c30] font-bold">{loan.student.name}</p>
                        <p className="text-[#43474e] text-xs">Kelas {loan.student.class}</p>
                      </td>
                      ...
                      <td className={`py-4 px-6 font-bold flex items-center gap-1.5 mt-2 border-0 ${isOverdue ? 'text-[#ba1a1a]' : 'text-[#43474e]'}`}>
                        {loan.returnedAt 
                      ...
                      {/* Tampilan Hitung Mundur & Denda (Khusus Tab Aktif) */}
                      {activeTab === "active" && (
                        <td className="py-4 px-6">
                          {isOverdue ? (
                            <div className="flex flex-col">
                              <span className="text-[#ba1a1a] text-xs font-bold flex items-center gap-1">
                                <AlertTriangle size={14} /> Terlambat {lateDays} Hari
                              </span>
                              <span className="text-[#93000a] text-[10px] font-black mt-1">
                                DENDA: Rp {totalDenda.toLocaleString('id-ID')}
                              </span>
                            </div>
                          ) : (
                            <span className="text-[#006a6a] text-xs font-bold px-2 py-1 bg-[#006a6a]/10 rounded-md">
                              {diffDays === 0 ? "Hari Terakhir!" : `Sisa ${diffDays} Hari`}
                            </span>
                          )}
                        </td>
                      )}
